...
def sync_sequences(folder) :
    ...
    # For double checking needs, export the full sequence
    if prm.export_verif :
        arr = np.asarray(full_sequence)
        np.save('./dev/test_regen.npy', arr, allow_pickle=True)
    print('\nVerification list :')
    print(verif_list)
            ...
            for u_theta in prm.thetas :
                ...
                for seq in sequences_contents :
                    if seq['sequence_theta'] == u_theta and seq['stimtype'] == 'MC' and seq['sequence_phase'] == phase and not prm.stack_TC : 
                        spikes_per_theta.append(seq['tot_spikes']-bsl)
...
def baseline_removal(folder) :
    ...
    for cluster_folder in tqdm(clusters_folders) :
        ...
        pre_fr_list = []
        for seq in sequences_contents :
            beg = int(seq['sequence_beg'])
            end = int(seq['sequence_beg'] + (delta_t * prm.fs))

            pre_spikes = spiketimes[np.where((spiketimes > beg) & (spiketimes < end))[0]]
            pre_fr_list.append(len(pre_spikes))
            
        np.save(folder_path + cluster_folder + '/baseline.npy', np.mean(pre_fr_list))
        np.save(folder_path + cluster_folder + '/baseline_all.npy', pre_fr_list)
...
def stat_baseline(folder) :
    '''
    For each btheta, compares how statistically different the prefered orientation activity
    is from the baseline. Prefered orientation activity is defined as the max for a trial,
    the baseline being the number of spikes before these trials
    
    The code is the same to TC calculating, except we're calculating the pre-spikes here
    then just reloading the TC arrays after for the comparison
    '''
    
    folder_path = './results/%s/' % folder
    clusters_folders = [file for file in os.listdir(folder_path) if os.path.isdir(os.path.join(folder_path, file))]
    
    print('\nCalculating baseline statistic...')
        ...
        for phase in prm.phases  :
            ...
            for u_btheta in prm.B_thetas:
                ...
                for u_theta in prm.thetas :
                    ...
                    for seq in sequences_contents :
                        if seq['sequence_theta'] == u_theta and seq['sequence_btheta'] == u_btheta and seq['sequence_phase'] == phase and seq['stimtype'] == 'MC' and not prm.stack_TC:
                            beg = int(seq['sequence_beg'] + (prm.baseline_beg * prm.fs))
                            end = int(seq['sequence_beg'] + (prm.baseline_end * prm.fs))
                            pre_spikes = spiketimes[np.where((spiketimes > beg) & (spiketimes < end))[0]]
                            spikes_per_thetabtheta.append(len(pre_spikes))
            ...
            baseline_array = np.asarray(TC_list_btheta)
            acti_array = np.load(folder_path + cluster_folder + '/%.3f_plot_MC_TC_nonmerged_all.npy' % phase)



            stats, pvals = [], []
            for i0, u_btheta in enumerate(prm.B_thetas) :

                evoked_cv = []
                for trial in range(0, acti_array.shape[-1]) :
                    arr = acti_array[i0, :, trial]
                    R = np.sum( (arr * np.exp(2j*prm.thetas)) / np.sum(arr) )
                    cirvar = 1 - np.abs(np.real(R))
                    evoked_cv.append(cirvar)
                    
                baseline_cv = []
                for trial in range(0, acti_array.shape[-1]) :
                    arr = baseline_array[i0, :, trial]
                    R = np.sum( (arr * np.exp(2j*prm.thetas)) / np.sum(arr) )
                    cirvar = 1 - np.abs(np.real(R))
                    baseline_cv.append(cirvar)

                stat, p = wilcoxon(evoked_cv, baseline_cv)
                stats.append(stat)
                pvals.append(p)
            
            np.save(folder_path + cluster_folder + '/%.3f_baseline_stats.npy'% phase, stats)
            np.save(folder_path + cluster_folder + '/%.3f_baseline_pvals.npy'% phase, pvals)
