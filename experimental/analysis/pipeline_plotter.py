...
def make_id(i, clusters_folders, folder_path, folder) :
    ...
    # Generate the mqps for quick visualisation, working on TC arrays
    #create_maps(cluster_folder = cluster_path)
    ...
    for i0, phase in enumerate(prm.phases) :
        ...
        #make_fit_psth(cluster_folder = cluster_path, phase = phase, pagenum = pagenum)
        #pagenum +=1
        
        #make_fit_psth_params(cluster_folder = cluster_path, phase = phase, pagenum = pagenum)
        #pagenum +=1
...
def make_pg1(folder, cluster_folder, phase, pagenum):
    ...
    try : 
        if cluster_info.putative_type == 'exc' :
            colwav = '#A62000'
        elif cluster_info.putative_type == 'inh' :
            colwav = '#0B61A4'
    except AttributeError : #unclassifiable waveform
        colwav = 'gray'
    
    mean_waveform = np.load(cluster_folder + '/waveform_mean.npy')
    std_waveform = np.load(cluster_folder + '/waveform_std.npy')
    ...
    try : 
        ...
        plt.text(0.13, .92, 'Putative type : Unclassifiable',
                 fontsize = 15, transform = plt.gcf().transFigure,
                 color = colwav)
...
def make_pg2(cluster_folder, phase, pagenum):
    ...
    pvals = np.load(cluster_folder + '/%.3f_baseline_pvals.npy' % phase)
    pvals_treshs = [.05, .005, .0001]
    ...
    for i, btheta in enumerate(unique_bthetas) :
        ...
        if pvals_treshs[1] < pvals[i] < pvals_treshs[0] :
            multiax.text(0, np.mean(mean_FR_per_btheta[i]) + i * 10, '*', fontsize = 12, fontweight = 'bold')
        elif pvals_treshs[2] < pvals[i] < pvals_treshs[1] :
            multiax.text(0, np.mean(mean_FR_per_btheta[i]) + i * 10, '**', fontsize = 12, fontweight = 'bold')
        elif pvals[i] < pvals_treshs[2] :
            multiax.text(0, np.mean(mean_FR_per_btheta[i]) + i * 10, '***', fontsize = 12, fontweight = 'bold')
...
# 
# --------------------------------------------------------------
def create_maps(cluster_folder) : 
    
    pagenum = 0
    for i0, btheta in enumerate(prm.B_thetas) :
        
        map_list = []
        for i1, phase in enumerate(prm.phases) :
            arr = np.load(cluster_folder + '/%.3f_plot_MC_TC_nonmerged_means.npy'% phase)
                
            mean_fr = arr[i0]  #we only take the i0 index to select one btheta and all thetas
            map_list.append(mean_fr)
            
        map_list = np.asarray(map_list)
        
        fig = plt.figure(figsize = (10,6))
        plt.imshow(map_list, origin = 'upper')
        plt.xlabel('Thetas')
        plt.ylabel('Phase')
        plt.title(' Btheta = %.4f -- Map of mean firing rate in TC intervals' % btheta)
        plt.colorbar()
        fig.savefig(cluster_folder + '/map%s.pdf' % pagenum)
        plt.close(fig) # We don't want display
            
        pagenum += 1
        
    pdfs = [cluster_folder + '/map%s.pdf' % x for x in np.arange(0,pagenum)]
    merger = PdfFileMerger()

    for pdf in pdfs:
        merger.append(pdf)
    
    merger.write(cluster_folder + 'maps.pdf')
    merger.close()
    
    for filename in glob.glob(cluster_folder + '/map*'):
        os.remove(filename) 
