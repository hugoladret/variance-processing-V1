...
def export_sequences_times() :
    ...
    for folder in prm.folder_list :
        ...
        signal = np.fromfile('./raw_data/%s/photodiode.bin' % folder, np.int16)
        #signal = np.load('./pipelines/%s/photodiode_v2.npy' % folder)
        ...
        np.save('./processed/%s/sequences_times.npy' % folder, sequences_times)
...
def get_peak_times(signal, beg_index, end_index,
    ...
    do_dubious_shit = True
    if do_dubious_shit and folder == 'April_C001' :
        sequences_times = []
        mean_duration = []
        for i in range(1152) :
            stim_beg = chunk_list[0][0] + i*(0.967*prm.fs)
            stim_end = chunk_list[0][0] + (i+1)*(0.967*prm.fs)
            sequences_times.append((stim_beg, stim_end))
            mean_duration.append(stim_end-stim_beg)
